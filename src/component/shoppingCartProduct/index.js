...
const ShoppingCartProduct = (prop) => {
  ...
  return (
    <Card sx={{ display: 'flex',padding:"20px",boxShadow: "none" }}>
      <img style={{width:"40%"}} src={item.url}/>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'row',disableRipple: true  }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center',flexDirection: 'column', padding:'20px',marginLeft:"10px",justifyContent:"space-around" }}>
        <AiOutlinePlusCircle onClick={() => dispatch(toIncreaseProductCount({id:item.id}))} size={30}/>
        <Typography variant="h7"  color="black" component="div" >
              {item.count}
          </Typography>
        <AiOutlineMinusCircle onClick={() => dispatch(toReduceProductCount({id:item.id}))} size={30}/>
        </Box>
      </Box>
